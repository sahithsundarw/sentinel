...
def test_reset_no_body_defaults_to_task1():
    r = client.post("/reset")
    assert r.status_code == 200
    # Default task is basic_threat_detection — prompts_remaining should be 167 (168 total, first served)
    obs = r.json()
    assert obs["prompts_remaining"] == 167
