...
def test_grader_task3_recovery_overblock_penalty():
    ...
    # total_weighted = -2.8, max_possible = 2.0, raw = -1.4 → clamped to _SCORE_MIN
    from app.grader import _SCORE_MIN
    assert score_all_refuse <= _SCORE_MIN + 1e-6, f"Refusing all 10 recovery turns should clamp to _SCORE_MIN, got {score_all_refuse}"
