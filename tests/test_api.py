...
def test_reset_no_body_defaults_to_task1():
    r = client.post("/reset")
    assert r.status_code == 200
    # Default task is basic_threat_detection — prompts_remaining should be 70 (71 eval, first served)
    obs = r.json()
    assert obs["prompts_remaining"] == 70
...
def test_step_done_true_on_last_step_task1():
    ...
    assert steps == 71  # 71 eval prompts (228 total, ~33% held-out)
...
def test_training_data_endpoint_returns_train_prompts():
    ...
    assert data["count"] == 157  # 157 train prompts (228 total, ~33% eval)
